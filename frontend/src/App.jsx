...
const safeAccess = (obj, path, defaultValue = null) => {
  try {
    // Handle both object.property and dictionary['property'] access patterns
    ...
    for (const key of keys) {
      result = result[key] !== undefined ? result[key] :
      (typeof result.get === 'function' ? result.get(key) : undefined);
...
function App() {
  ...
  const sendTextMessage = async (e) => {
    ...
    const assistantMsg = {
      id: Date.now() + 1,
      type: 'assistant',
      content: safeAccess(response.data, 'response', 'No response received'),
      confidence: safeAccess(response.data, 'confidence', 0.5),
      relationships: safeAccess(response.data, 'relationships', {}),
      processing_time: safeAccess(response.data, 'processing_time', 0),
      agents_consulted: safeAccess(response.data, 'agents_consulted', []),
      tools_used: safeAccess(response.data, 'tools_used', []),
      reasoning_approach: safeAccess(response.data, 'reasoning_approach', ''),
      timestamp: new Date()
    };
  ...
  const sendVoiceMessage = async (audioBlob) => {
    ...
    try {
      ...
      setMessages(prev => prev.map(msg => 
        msg.id === voiceMsg.id ? { ...msg, content: `🎤 "${safeAccess(response.data, 'query', 'Voice message')}"` } : msg
      ));
  ...
  const handleFileUpload = async (file, datasetName = 'uploaded_documents') => {
    ...
    try {
      ...
      // Safe access to response message
      const message = safeAccess(response.data, 'message', 'File uploaded successfully, but no details returned');
  ...
  const handleUrlUpload = async (url, category = 'general', datasetName = 'web_content') => {
    ...
    try {
      ...
      // Safe access to response message
      const message = safeAccess(response.data, 'message', 'URL processed successfully, but no details returned');
      ...
      <div className="main-content">
        ...
        <div className="messages">
          ...
          {messages.map((msg) => (
            ...
            <div className="message-content">
              <p>{msg.content}</p>
              {Array.isArray(msg.agents_consulted) && msg.agents_consulted.length > 0 && (
                <div className="agents-info">
                  <div className="agents-consulted">
                    ...
                    <div className="agents-list">
                      {msg.agents_consulted.map((agent, idx) => (
                        <span key={idx} className="agent-badge">{agent ? agent.replace('_', ' ') : 'Unknown'}</span>
                      ))}
                    </div>
                  </div>
                  ...
                  {Array.isArray(msg.tools_used) && msg.tools_used.length > 0 && (
              ...
              {msg.relationships && typeof msg.relationships === 'object' && Object.keys(msg.relationships).length > 0 && (
                <div className="relationships">
                  <Database className="relationship-icon" />
                  <span>Found {Object.values(msg.relationships).reduce((a, b) => a + (b.count || 0), 0)} related concepts</span>
                </div>
              )}
